...
import 'tasks_edit_model.dart';
export 'tasks_edit_model.dart';

/// сгенерируй окно создания и редактирования и удаления задачи.
///
/// на русском. у задачи есть название описание статус приоритет и категория
class TasksEditWidget extends StatefulWidget {
  const TasksEditWidget({
    super.key,
    this.categorytask,
    String? prioritytask,
    String? statustasks,
    this.taskid,
  })  : this.prioritytask = prioritytask ?? 'Низкий',
        this.statustasks = statustasks ?? 'Новая';

  final String? categorytask;
  final String prioritytask;
  final String statustasks;
  final int? taskid;
...
class _TasksEditWidgetState extends State<TasksEditWidget> {
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => TasksEditModel());
                          ...
                          children: [
                            FFButtonWidget(
                              onPressed: () async {
                                ...
                                if (widget.taskid != null ? true : true) {
                                  ...
                                  await TaskRecord.collection.doc().set({
                                    ...createTaskRecordData(
                                      ...
                                      title: _model.textController1.text,
                                      description: _model.textController2.text,
                                      category: widget.categorytask,
                                      priority: widget.prioritytask,
                                      status: widget.statustasks,
                                      taskid: random_data.randomInteger(0, 10),
                                    ),
                                    ...mapToFirestore(
                                      {
                                        'created_time':
